...
def _settings(tmp_path: Path, **overrides: object) -> Settings:
    ...
    overrides.setdefault("critic", "fake")  # 033: `api` is the default (10.2)
    return Settings(
...
def test_providers_in_use_follow_the_config(tmp_path: Path) -> None:
    ...
    # 033: the critic is its own provider, priced like the planner; `fake` bills nothing.
    criticised = _settings(tmp_path, planner="fake", critic="api")
    assert ledger.providers_in_use(criticised) == {"critic"}
    assert ledger.REQUIRED_UNITS["critic"] == ledger.REQUIRED_UNITS["planner"]
    full = _settings(
        tmp_path, planner="api", image_gen="gemini", transcriber="groq", relevance_judge="api",
        critic="api",
    )  # fmt: skip
    assert ledger.providers_in_use(full) == {"planner", "gemini", "groq", "judge", "critic"}
