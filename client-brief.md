...
**What I want — the destination.** A web page where a creator uploads a raw recording (their face + voice), writes a short brief for the Short (topic, angle, must-say facts, hook wish; free text, a few lines, not one sentence), picks a style from `styles/` (or names it in plain words, e.g. "explainer, 55 seconds, Hindi captions, energetic" or "hi-tech animated for a product fact"), optionally attaches reference images or clips, and presses one button. They get back a publish-ready Short that looks cut by a top editor:
...
- Script-matched B-roll for every beat — relevant images or clips with motion (Ken Burns, zoom-to-point, framed archival cards, maps, charts, stamps, lower-thirds) — from the user's supplied images (which outrank sourced ones and are logged as owner-supplied), rights-safe web sources, or AI-generated images.
...
**Hard constraints.**
- One page, one button; no terminal. The four job inputs (raw video, written brief, style choice, optional reference images) are captured, validated and stored per job, and the brief and references reach the planner. Shareable by link; passcode in v1; uploads deleted after 24 h.
